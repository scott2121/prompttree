...
- writes `examples/sort/output/run.txt` with the promoted winner and benchmark summary
- writes per-round lineage context JSON files under `examples/sort/output/lineage-contexts/`
...
This example uses `examples/sort/` itself as the PromptTree workspace. Each run recreates `prompting/` and `.prompttree/` inside that example directory, while `examples/sort/output/` holds generated benchmark artifacts. The checked-in SVG files in `output/` are sample visualization artifacts from an earlier run, not files regenerated by the current script.
