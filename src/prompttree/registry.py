...
from .models import ExperimentArm, PromptExperiment, PromptFamily, PromptVersion
...
class Registry:
    ...
    def resolve_version(self, family_id: str, version_ref: str) -> PromptVersion:
        ...
        version_id = family.current_version if version_ref == "current" else version_ref
    ...
    def get_active_experiment(self, family_id: str) -> Optional[PromptExperiment]:
        for experiment in self.list_experiments(family_id=family_id):
            if experiment.status == "running":
                return experiment
        return None

    def create_family(self, family_id: str, name: str, description: str, current_version: str, artifact_kind: str, stage: str = "") -> Path:
    ...
    def _load_family(self, family_dir: Path) -> PromptFamily:
        ...
        return PromptFamily(
            ...
            metadata={k: v for k, v in data.items() if k not in {"id", "name", "description", "current_version", "artifact_kind", "stage"}},
        )
